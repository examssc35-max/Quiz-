...
class OpenRouterProvider(
    ...
    override suspend fun testConnection(config: AIConfig): Result<String> = withContext(Dispatchers.IO) {
        val apiKey = config.effectiveApiKey
        if (apiKey.isBlank()) {
            return@withContext Result.failure(IllegalStateException("API Key is missing. Please enter your OpenRouter API Key."))
        }
        ...
        try {
            client.newCall(httpRequest).execute().use { response ->
                ...
                if (response.isSuccessful) {
                    Result.success("Connection successful! OpenRouter ($model) responded in ${duration}ms.")
                } else {
                    val code = response.code
                    val err = sanitizeError(response.body?.string().orEmpty())
                    val helpfulHint = when (code) {
                        401 -> "Invalid OpenRouter API Key."
                        404 -> "Model '$model' was not found on OpenRouter."
                        429 -> "Rate limit or credit limit reached on OpenRouter."
                        else -> err
                    }
